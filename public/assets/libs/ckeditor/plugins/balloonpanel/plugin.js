...
(function() {
    var q = !1;
    CKEDITOR.plugins.add("balloonpanel", { init: function() { q || (CKEDITOR.document.appendStyleSheet(this.path + "skins/" + CKEDITOR.skin.name + "/balloonpanel.css"), q = !0) } });
    ...
    CKEDITOR.ui.balloonPanel.prototype = {
        ...
        attach: function() {
            ...
            return function(u, h) {
                ...
                l = this.editor.window.getFrame();
                t = this.editor.editable();
                r = t.isInline();
                var p = this.getWidth(),
                    d = this.getHeight(),
                    f = this._getAbsoluteRect(u),
                    m = this._getAbsoluteRect(r ? t : l),
                    g = c.getViewPaneSize(),
                    k = c.getScrollPosition(),
                    g = { top: Math.max(m.top, k.y), left: Math.max(m.left, k.x), right: Math.min(m.right, g.width + k.x), bottom: Math.min(m.bottom, g.height + k.y) };
                ...
                this._adjustElementRect(f, r ? g : m);
                var m = p * d,
                    f = this._getAlignments(f, p, d),
                    e, n;
                for (n in f) {
                    k = b(f[n].top, f[n].left, p, d);
                    k = f[n].areaDifference = m - a(k, g);
                    if (0 === k) { e = n; break }
                    e || (e = n);
                    k < f[e].areaDifference && (e =
                        n)
                }
                p = (d = this.parts.panel.getAscendant(function(a) { return a instanceof CKEDITOR.dom.document ? !1 : "static" !== a.getComputedStyle("position") })) ? parseInt(d.getComputedStyle("margin-left"), 10) : 0;
                d = d ? parseInt(d.getComputedStyle("margin-top"), 10) : 0;
                this.move(f[e].top - d, f[e].left - p);
                e = e.split(" ");
        ...
        getWidth: function() { return "auto" === this.rect.width ? this.parts.panel.getClientRect().width : this.rect.width },
        ...
        setTriangle: function(a, b) {
            var c = this.parts.triangleOuter,
                l = this.parts.triangleInner;
            this.triangleSide && (c.removeClass("cke_balloon_triangle_" + this.triangleSide), c.removeClass("cke_balloon_triangle_align_" + this.triangleAlign), l.removeClass("cke_balloon_triangle_" +
                this.triangleSide));
            this.triangleSide = a;
            this.triangleAlign = b;
            c.addClass("cke_balloon_triangle_" + a);
            c.addClass("cke_balloon_triangle_align_" + b);
            l.addClass("cke_balloon_triangle_" + a)
        },
        ...
        addShowListener: function(a) {
            ...
            var c = this;
            return { removeListener: function() { c.removeShowListener(b) } }
        },
        ...
        _getAbsoluteRect: function(a) {
            var b = a.getClientRect(),
                c = CKEDITOR.document.getWindow().getScrollPosition(),
                l = this.editor.window.getFrame();
            this.editor.editable().isInline() || a.equals(l) ? (b.top += c.y, b.left += c.x) : (a = l.getClientRect(), b.top = a.top + b.top + c.y, b.left = a.left + b.left + c.x);
