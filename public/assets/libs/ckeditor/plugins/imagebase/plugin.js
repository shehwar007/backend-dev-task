...
(function() {
    function m(c) {
        ...
        if (c.focusManager.hasFocus) { if (a.focused) return a.focused; if (b instanceof CKEDITOR.plugins.widget.nestedEditable) return a.getByElement(b) }
    }
    ...
    function p() {
        n.call(this, '\x3cdiv class\x3d"cke_loader"\x3e\x3cdiv class\x3d"cke_bar" styles\x3d"transition: width ' + q / 1E3 + 's"\x3e\x3c/div\x3e\x3c/div\x3e');
        this.bar = this.wrapper.getFirst()
    }
    var r = !1,
        u = {
            caption: function() {
                function c(b) { b.parts.caption.data("cke-caption-placeholder", !1) }
                ...
                return {
                    setUp: function(b) {
                        function a(d) {
                            d = (d = "blur" === d.name ? b.elementPath() : d.data.path) ?
                                d.lastElement : null;
                            var e = m(b),
                                h = b.widgets.getByElement(b.editable().findOne("figcaption[data-cke-caption-active]"));
                            if (!b.filter.check("figcaption")) return CKEDITOR.tools.array.forEach(c, function(b) { b.removeListener() });
                            e && l(e, "caption") && e._refreshCaption(d);
                            h && l(h, "caption") && h._refreshCaption(d)
                        }
                        var c = [];
                    ...
                    init: function() {
                        if (this.editor.filter.check("figcaption")) {
                            if (!this.parts.caption) {
                                var b = this.parts,
                                    a = this.element,
                                    c = a.getDocument().createElement("figcaption");
                                a.append(c);
                    ...
                    _refreshCaption: function(b) {
                        var d = m(this.editor) === this,
                            f = this.parts.caption,
                            g = this.editables.caption;
                        if (d) g.getData() || b.equals(f) ? (!b || b.equals(f) && b.data("cke-caption-placeholder")) && c(this) : this.parts.caption.data("cke-caption-placeholder", this.editor.lang.imagebase.captionPlaceholder), a(f, !0);
                        else if (!this.editables.caption.getData() || this.parts.caption.data("cke-caption-placeholder")) c(this),
                            a(f, !1)
                    }
                }
            }(),
            upload: function() {
                var c = {
                    ...
                    setUp: function(a, b) {
                        a.on("paste", function(d) {
                            ...
                            if (!a.readOnly && ("drop" === f || "paste" === f && e)) {
                                ...
                                for (var k = 0; k < e; k++) f = g.getFile(k), CKEDITOR.fileTools.isTypeSupported(f, b.supportedTypes) && h.push(f);
                    ...
                    _isLoaderDone: function(a) { return a.xhr && 4 === a.xhr.readyState },
                    ...
                    _beginUpload: function(a, b) {
                        ...
                        function f() { c();!1 !== a.fire("uploadFailed", { loader: b }) && a.editor.widgets.del(a) }
                        var g = {
                                uploaded: function() {
                                    c();
                    ...
                    setUp: function(d) {
                        d.plugins.link && (d.contextMenu && c(d), d.on("dialogShow", function(b) {
                            var c = m(d),
                                e = b.data,
                                h, k;
                            c && l(c, "link") && "link" === e._.name && (h = e.getContentElement("info", "linkDisplayText").getElement().getParent().getParent(),
                                e.setupContent(c.data.link || {}), h.hide(), k = e.once("ok", a(b, e, c), null, null, 9), e.once("hide", function() {
                                    k.removeListener();
                                    h.show()
                                }))
                        ...
                        }), b(d, "refresh", function(a, b) { a.setState(b.parts.link ? CKEDITOR.TRISTATE_OFF : CKEDITOR.TRISTATE_DISABLED) }))
                    },
                    data: function(a) {
                        var b = this.editor,
                            c = a.data.link,
                            e = this.element.findOne("img");
                        "undefined" === typeof c && this.parts.link && this.setData("link", CKEDITOR.plugins.link.parseLinkAttributes(this.editor,
                            this.parts.link));
    ...
    n.prototype = {
        ...
        bindLoader: function(c) {
            ...
            b.push(c.once("uploaded",
                a));
            b.push(c.once("error", a))
        }
    };
    p.prototype = new n;
    p.prototype.updated = function(c) {
        c = Math.round(100 * c);
        c = Math.max(c, 0);
        c = Math.min(c, 100);
