...
(function() {
    function f(a) {
        ...
        for (var c = a.count(), b, d = 0; d < c; d++) b = a.getItem(d),
            function(a) {
    ...
    CKEDITOR.plugins.add("adobeair", {
            ...
            CKEDITOR.env.air && (CKEDITOR.dom.document.prototype.write = CKEDITOR.tools.override(CKEDITOR.dom.document.prototype.write, function(a) {
                ...
                return function(b) {
                    if (this.getBody()) {
                        var d = this,
                            f = this.getHead();
                        b = b.replace(/(<style[^>]*>)([\s\S]*?)<\/style>/gi, function(a, b, d) { c(f, "style", b, d); return "" });
                        b = b.replace(/<base\b[^>]*\/>/i, function(b) { c(f, "base", b); return "" });
                        b = b.replace(/<title>([\s\S]*)<\/title>/i, function(b, a) { d.$.title = a; return "" });
        ...
        init: function(a) {
            CKEDITOR.env.air && (a.on("uiReady", function() {
                f(a.container);
                a.on("elementsPathUpdate", function(a) { f(a.data.space) })
            }), a.on("contentDom", function() { a.document.on("click", function(a) { a.data.preventDefault(!0) }) }))
        }
    })
})();